      ...
      if (hasBmc && generableTypes.includes('bmc_analysis')) {
        try {
          ...
          const bmcAnalRow = await db.prepare(
            "SELECT content, score FROM entrepreneur_deliverables WHERE user_id = ? AND type = 'bmc_analysis' ORDER BY created_at DESC LIMIT 1"
          ).bind(userId).first() as any
      ...
      if (hasSic && generableTypes.includes('sic_analysis')) {
        try {
          ...
          const sicAnalRow = await db.prepare(
            "SELECT content, score FROM entrepreneur_deliverables WHERE user_id = ? AND type = 'sic_analysis' ORDER BY created_at DESC LIMIT 1"
          ).bind(userId).first() as any
