...
import { generateFullBmcDeliverable, generateFullBmcDeliverableFallback, regenerateBmcHtmlFromDbAnalysis, type BmcDeliverableData, type KBContextForBmc } from './bmc-deliverable-engine'
import { generateFullSicDeliverable, generateFullSicDeliverableFallback, renderSicDeliverableFromAnalyst, regenerateSicHtmlFromDbAnalysis, type SicDeliverableData, type SicAnalystDeliverableInput } from './sic-deliverable-engine'
    ...
    if (coachUserId) {
      ...
      deliverables = await c.env.DB.prepare(`
        ...
        INNER JOIN (
          ...
          WHERE status = 'generated' AND (
            (user_id = ? AND NOT (generated_by IN ('coach','coach_mirror') AND visibility = 'private'))
             OR (coach_user_id = ? AND visibility = 'shared')
          )
          GROUP BY type
        ) latest 
        ON ed.type = latest.type AND ed.version = latest.max_version
        WHERE ed.status = 'generated' AND (
          (ed.user_id = ? AND NOT (ed.generated_by IN ('coach','coach_mirror') AND ed.visibility = 'private'))
      ...
      deliverables = await c.env.DB.prepare(`
        ...
        INNER JOIN (SELECT type, MAX(version) as max_version FROM entrepreneur_deliverables 
          WHERE user_id = ? AND status = 'generated' AND NOT (generated_by IN ('coach','coach_mirror') AND visibility = 'private')
          GROUP BY type) latest 
        ON ed.type = latest.type AND ed.version = latest.max_version
        WHERE ed.user_id = ? AND ed.status = 'generated' AND NOT (ed.generated_by IN ('coach','coach_mirror') AND ed.visibility = 'private')
...
entrepreneurRoutes.post('/api/ai/generate-all', async (c) => {
  try {
    ...
    // ═══ CRITICAL: Invalidate ALL previous deliverables ═══
    // When a new generation starts (possibly new company), OLD deliverables must NOT be shown.
    // Mark all previous deliverables as 'ancien' so they won't be served.
    // Only the new generation's deliverables (status='generated') will be displayed.
    try {
      await db.prepare(
        "UPDATE entrepreneur_deliverables SET status = 'ancien' WHERE user_id = ? AND status = 'generated'"
      ).bind(userId).run()
      console.log(`[Generate-All] ♻️ Invalidated all previous deliverables for user ${userId}`)
    } catch (invErr: any) {
      console.error(`[Generate-All] ⚠️ Failed to invalidate old deliverables: ${invErr.message}`)
    }
...
entrepreneurRoutes.post('/api/ai/pipeline/step', async (c) => {
  ...
  try {
    ...
    // ════════════════════════════════════════════════════════════════
    // STEP: bmc_html — Generate BMC HTML from DB analysis (no extra Claude call)
    // ════════════════════════════════════════════════════════════════
    if (step === 'bmc_html') {
      console.log(`[Pipeline:bmc_html] Starting for job ${jobId}`)
      await updateJobProgress(db, jobId, 'Génération: BMC HTML...')

      const { result, iterationId: iterId, companyName: cn } = ctx
      const companyName = cn || userName

      if (hasBmc && generableTypes.includes('bmc_analysis')) {
        try {
          // Load already-computed bmc_analysis from DB (generated by agent_bmc step)
          const bmcAnalRow = await db.prepare(
            "SELECT content, score FROM entrepreneur_deliverables WHERE user_id = ? AND type = 'bmc_analysis' AND status = 'generated' ORDER BY created_at DESC LIMIT 1"
          ).bind(userId).first() as any

          if (bmcAnalRow?.content) {
            const bmcAnalysis = JSON.parse(bmcAnalRow.content)
            const bmcScore = bmcAnalRow.score || bmcAnalysis.score || 0
            console.log(`[Pipeline:bmc_html] Loaded bmc_analysis: score=${bmcScore}, blocks=${bmcAnalysis.blocks?.length || 0}`)

            // Load PME data for company info enrichment
            const pmeDataRow = await db.prepare(
              "SELECT content FROM entrepreneur_deliverables WHERE user_id = ? AND type = 'framework_pme_data' AND status = 'generated' ORDER BY version DESC LIMIT 1"
            ).bind(userId).first() as any
            let sector = '', location = ''
            if (pmeDataRow?.content) {
              try {
                const pd = JSON.parse(pmeDataRow.content)
                sector = pd.sector || pd.activite || ''
                location = pd.location || pd.ville || ''
              } catch {}
            }

            // Generate HTML directly from the analysis — no need for document re-parsing
            const bmcHtml = regenerateBmcHtmlFromDbAnalysis(bmcAnalysis, {
              companyName,
            ...
            console.log(`[Pipeline:bmc_html] BMC HTML generated: ${bmcHtml.length} chars`)
            ...
            `).bind(bmcHtmlId, userId, bmcHtml, bmcScore, newVersion, iterId).run()
            ctx.agentsUsed.push('bmc_deliverable_engine:html')
          } else {
            console.warn('[Pipeline:bmc_html] No bmc_analysis found in DB — skipping HTML generation')
          }
        } catch (err: any) {
    ...
    // ════════════════════════════════════════════════════════════════
    // STEP: sic_html — Generate SIC HTML from DB analysis (no extra Claude call)
    // ════════════════════════════════════════════════════════════════
    if (step === 'sic_html') {
      ...
      if (hasSic && generableTypes.includes('sic_analysis')) {
        try {
          // Load already-computed sic_analysis from DB (generated by agent_sic step)
          const sicAnalRow = await db.prepare(
            "SELECT content, score FROM entrepreneur_deliverables WHERE user_id = ? AND type = 'sic_analysis' AND status = 'generated' ORDER BY created_at DESC LIMIT 1"
          ).bind(userId).first() as any

          if (sicAnalRow?.content) {
            let sicAnalysis = JSON.parse(sicAnalRow.content)
            const sicScore = sicAnalRow.score || sicAnalysis.score || 0
            
            // Unwrap JSON schema wrapper if present
            if (sicAnalysis.properties && !sicAnalysis.pillars) {
              sicAnalysis = sicAnalysis.properties
              console.log('[Pipeline:sic_html] Unwrapped JSON schema wrapper')
            }
            console.log(`[Pipeline:sic_html] Loaded sic_analysis: score=${sicScore}, pillars=${sicAnalysis.pillars?.length || 0}`)
            ...
            // Generate HTML directly from the analysis
            const sicHtml = regenerateSicHtmlFromDbAnalysis(sicAnalysis, {
              companyName,
              entrepreneurName: userName,
              sector,
              location,
            ...
            console.log(`[Pipeline:sic_html] SIC HTML generated: ${sicHtml.length} chars`)
            ...
            `).bind(sicHtmlId, userId, sicHtml, sicScore, newVersion, iterId).run()
            ctx.agentsUsed.push('sic_deliverable_engine:html')
          } else {
            console.warn('[Pipeline:sic_html] No sic_analysis found in DB — skipping HTML generation')
          }
        } catch (err: any) {
          ...
          if (pmeData) {
            ...
            try {
              let bmcDel = await db.prepare(
                "SELECT content FROM entrepreneur_deliverables WHERE user_id = ? AND type = 'bmc_analysis' AND status = 'generated' ORDER BY version DESC LIMIT 1"
              ...
              if (!bmcContent) {
                bmcDel = await db.prepare(
                  "SELECT content FROM entrepreneur_deliverables WHERE user_id = ? AND type = 'bmc_html' AND status = 'generated' ORDER BY version DESC LIMIT 1"
                ).bind(userId).first<any>()
                if (bmcDel?.content && bmcDel.content.length > 100) {
        ...
        try {
          const loadDeliv = async (type: string) => {
            const row = await db.prepare(
              "SELECT content FROM entrepreneur_deliverables WHERE user_id = ? AND type = ? AND status = 'generated' ORDER BY version DESC LIMIT 1"
    ...
    if (step === 'postprocess') {
      ...
      try {
        const bmcHtmlRow = await db.prepare(
          "SELECT content FROM entrepreneur_deliverables WHERE user_id = ? AND type = 'bmc_html' AND status = 'generated' ORDER BY created_at DESC LIMIT 1"
        ).bind(userId).first() as any
        ...
        if (bmcHtmlRow?.content) {
          ...
          if (undocCount > 0 || nonRensCount > 3) {
            ...
            try {
              ...
              const { regenerateBmcHtmlFromDbAnalysis: regenBmc } = await import('./bmc-deliverable-engine')
              const bmcAnalParsed = bmcAnalJson?.content ? JSON.parse(bmcAnalJson.content) : null
              if (bmcAnalParsed) {
                const newBmcHtml = regenBmc(bmcAnalParsed, fixCompanyData)
      ...
      try {
        const sicAnalRow = await db.prepare(
          "SELECT content FROM entrepreneur_deliverables WHERE user_id = ? AND type = 'sic_analysis' AND status = 'generated' ORDER BY version DESC LIMIT 1"
        ).bind(userId).first() as any
        ...
        if (sicAnalRow?.content) {
          try {
            ...
            const sicHtmlRow = await db.prepare(
              "SELECT content FROM entrepreneur_deliverables WHERE user_id = ? AND type = 'sic_html' AND status = 'generated' ORDER BY created_at DESC LIMIT 1"
            ).bind(userId).first() as any
            ...
            if (sicHtmlRow?.content) {
              ...
              if (needsRegen) {
                try {
                  ...
                  const { regenerateSicHtmlFromDbAnalysis: regenSic } = await import('./sic-deliverable-engine')
                  const sicAnalParsed = wasWrapped ? JSON.parse(sicAnalRow.content).properties || sicJson : sicJson
                  const newSicHtml = regenSic(sicAnalParsed, fixCompanyData)
      ...
      try {
        const bpCheck = await db.prepare(
          "SELECT content, LENGTH(content) as len FROM entrepreneur_deliverables WHERE user_id = ? AND type = 'business_plan' AND status = 'generated' ORDER BY version DESC LIMIT 1"
        ).bind(userId).first() as any
        if (bpCheck?.content) {
      ...
      try {
        const ovoCheck = await db.prepare(
          "SELECT content, LENGTH(content) as len FROM entrepreneur_deliverables WHERE user_id = ? AND type = 'plan_ovo' AND status = 'generated' ORDER BY version DESC LIMIT 1"
        ).bind(userId).first() as any
        if (ovoCheck?.content) {
      ...
      try {
        const bpDeliv = await db.prepare(
          "SELECT content, score FROM entrepreneur_deliverables WHERE user_id = ? AND type = 'business_plan' AND status = 'generated' ORDER BY version DESC LIMIT 1"
        ).bind(userId).first() as any
        if (bpDeliv?.content) {
      ...
      try {
        const ovoDeliv = await db.prepare(
          "SELECT content, score FROM entrepreneur_deliverables WHERE user_id = ? AND type = 'plan_ovo' AND status = 'generated' ORDER BY version DESC LIMIT 1"
        ).bind(userId).first() as any
        if (ovoDeliv?.content) {
          const pmeId = `pme_${userId}`
...
entrepreneurRoutes.get('/api/deliverable/:type', async (c) => {
  try {
    ...
    const deliverable = await c.env.DB.prepare(`
      SELECT * FROM entrepreneur_deliverables WHERE user_id = ? AND type = ? AND status = 'generated' ORDER BY version DESC LIMIT 1
  ...
  try {
    ...
    if (!pmeData) {
      const frameworkDel = await c.env.DB.prepare(
        "SELECT content FROM entrepreneur_deliverables WHERE user_id = ? AND type = 'framework' AND status = 'generated' ORDER BY version DESC LIMIT 1"
      ).bind(payload.userId).first() as any
...
entrepreneurRoutes.get('/preview/:userId/:type', async (c) => {
  try {
    ...
    const deliverable = await c.env.DB.prepare(
      "SELECT * FROM entrepreneur_deliverables WHERE user_id = ? AND type = ? AND status = 'generated' AND NOT (generated_by IN ('coach','coach_mirror') AND visibility = 'private') ORDER BY version DESC LIMIT 1"
    ).bind(userId, dtype).first() as any
    if (!deliverable) return c.text('Livrable non encore généré ou en cours de régénération', 404)
...
entrepreneurRoutes.get('/deliverable/:type', async (c) => {
  try {
    ...
    // Get latest deliverable of this type (only 'generated', not 'ancien')
    const deliverable = await c.env.DB.prepare(`
      SELECT * FROM entrepreneur_deliverables WHERE user_id = ? AND type = ? AND status = 'generated' ORDER BY version DESC LIMIT 1
    ...
    if (dtype === 'sic_analysis') {
      ...
      const sicHtml = await c.env.DB.prepare(
        "SELECT content FROM entrepreneur_deliverables WHERE user_id = ? AND type = 'sic_html' AND status = 'generated' ORDER BY version DESC LIMIT 1"
      ).bind(payload.userId).first() as any
    ...
    if (dtype === 'framework') {
      const fwHtml = await c.env.DB.prepare(
        "SELECT content FROM entrepreneur_deliverables WHERE user_id = ? AND type = 'framework_html' AND status = 'generated' ORDER BY version DESC LIMIT 1"
      ).bind(payload.userId).first() as any
    ...
    if (dtype === 'diagnostic') {
      const diagHtml = await c.env.DB.prepare(
        "SELECT content FROM entrepreneur_deliverables WHERE user_id = ? AND type = 'diagnostic_html' AND status = 'generated' ORDER BY version DESC LIMIT 1"
      ).bind(payload.userId).first() as any
    ...
    // Plan Financier OVO — serve the actual plan_ovo JSON rendered, NOT inputs_html
    if (dtype === 'plan_ovo') {
      // The plan_ovo content is already loaded in `content` from the deliverable query above
      // Fall through to generic rendering below which handles plan_ovo at line ~3824
      // (inputs_html is a separate deliverable shown via 'inputs' type, not plan_ovo)
    }
...
entrepreneurRoutes.get('/entrepreneur', async (c) => {
  try {
    ...
    const diagJsonRow = await c.env.DB.prepare(
      "SELECT content FROM entrepreneur_deliverables WHERE user_id = ? AND type = 'diagnostic' AND status = 'generated' ORDER BY version DESC LIMIT 1"
    ).bind(payload.userId).first() as any
